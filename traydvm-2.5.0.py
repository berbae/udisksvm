...
gitem = Gio.MenuItem.new_section("   " + tooltip_header + "   ", menucontent)
...
def setup_tooltip(status_icon, x, y, keyboard_mode, tooltip):
    mountpoints = ifilesystem.get_cached_property('MountPoints')
    mountpoints = mountpoints.get_bytestring_array()
...
# Generate the tooltip text using the above setup_tooltip() function
trayicon.connect('query-tooltip', setup_tooltip)
