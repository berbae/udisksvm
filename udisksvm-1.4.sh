...
#	echo "blank : ---$blank---"
	case $event in
		...
		if [[ "$systeminternal" == "0" ]] && [[ "$usage" == "filesystem" ]] && [[ "$partition" == "1" ]]; then
			...
			(exec traydevice -c $TRAYCONF $devpath &>/dev/null) &
		    fi
		fi
		;;
	    job-changed)
		...
		if [[ "$systeminternal" == "0" ]] && [[ "$opticaldisc" == "1" ]] && [[ "$ismounted" == "0" ]]; then
			...
			if [[ "$usage" == "filesystem" ]] && [[ "$numaudiotracks" == "0" ]]; then
			    if ! pgrep -f "traydevice -c $TRAYCONF $devpath" &>/dev/null; then
				if [[ "$blank" == "0" ]]; then
				    udisks --mount $devpath --mount-options ro,noatime,nodiratime,noexec,nodev &>/dev/null
				fi
				(exec traydevice -c $TRAYCONF $devpath &>/dev/null) &
			    fi
			fi
		    else
			pkill -f "traydevice -c $TRAYCONF $devpath"
		    fi
		fi
		;;
	esac
